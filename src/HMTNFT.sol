...
contract HMT_NFT is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(address _usdt, address _ownerWallet) ERC721("HMT Reward NFT", "HMT-NFT") Ownable(msg.sender) {
        ...
        tiers[1] = Tier(1000 * 1e18, 7000, 0, 0);
        tiers[2] = Tier(2500 * 1e18, 5000, 0, 0);
        tiers[3] = Tier(5000 * 1e18, 2500, 0, 0);
        tiers[4] = Tier(10000 * 1e18, 500, 0, 0);
        tiers[5] = Tier(25000 * 1e18, 100, 0, 0);
        tiers[6] = Tier(50000 * 1e18, 10, 0, 0);
        tiers[7] = Tier(100000 * 1e18, 1, 0, 0);
    }
    ...
    function claimOwnerAllocation() external onlyOwner {
        ...
        for (uint8 i = 1; i <= 7; i++) {
            ...
            uint256 totalAllocation = (i <= 5) ? (t.maxSupply / 10) : t.maxSupply; 
            uint256 remainingForThisTier = totalAllocation - t.ownerMinted;

            if (remainingForThisTier > 0) {
                uint256 toMint = remainingForThisTier;
                if (mintedThisTx + toMint > batchLimit) { toMint = batchLimit - mintedThisTx; }
                ...
                mintedThisTx += toMint;
                if (mintedThisTx >= batchLimit) { break; }
            }
        }
    ...
    function getUserTokens(address _user) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < tokenCount; i++) { tokens[i] = tokenOfOwnerByIndex(_user, i); }
        return tokens;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, uint256(tier).toString(), ".json")) : "";
    }
    ...
    function buyNFT(address to, uint8 _tier) external {
        ...
        emit NFTBought(to, tokenId, _tier);
    }
    ...
    function getNFTTier(uint256 tokenId) external view returns (uint8) { return tokenTier[tokenId]; }
    function getTierPrice(uint8 _tier) external view returns (uint256) { return tiers[_tier].price; }
